...
void MyGLWidget::initializeGL ()
{
  ...
  pos_foco_Loc = glGetUniformLocation (program->programId(), "posFocusSCO");
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) {
  ...
  case Qt::Key_K: 
      if (posicion_foco_escena.x > -5) 
        posicion_foco_escena = glm::vec3 (posicion_foco_escena.x - 0.25, posicion_foco.y, posicion_foco.z);
      break;
  case Qt::Key_L: 
      if (posicion_foco_escena.x < 5) 
        posicion_foco_escena = glm::vec3 (posicion_foco_escena.x + 0.25, posicion_foco.y, posicion_foco.z);
      break;
  case Qt::Key_F:
      foco_en_camara = !foco_en_camara;    
      std::cout << foco_en_camara << std::endl;
      pasarInfoFoco();  
      break;
...
void MyGLWidget::pasarInfoFoco()
{
  ...
  if (foco_en_camara) {
    posicion_foco = glm::vec3 (1., 1., 1.);
    glUniform3fv(pos_foco_Loc, 1, &posicion_foco[0]);
  }
  else {
    posicion_foco = glm::vec3 (View * glm::vec4(posicion_foco_escena, 1.));
    glUniform3fv(pos_foco_Loc, 1, &posicion_foco[0]);
  }
