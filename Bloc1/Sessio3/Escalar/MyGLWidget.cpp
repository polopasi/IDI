...
void MyGLWidget::modelTransform () {
  ...
  glm::mat4 TG (1.0);       //Matriz transformación, inicialmente identidad.


  // retorna el producte de TG per una matriu que fa una traslación en tx, ty
  TG = glm::translate (TG, glm::vec3 (tx, ty, 0.0));

  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);

  glUniform1f(scaleLoc, scale); //Le pasamos el valor de scale al uniform scaleLoc
...
{  glm::vec3 Vertices[3];  // Tres vèrtexs amb X, Y i Z
  ...
  //^^^ asocia el vertexLoc al VBO activo
...
void MyGLWidget::carregaShaders()
{
  ...
  //los GetAtribb/GetUniform son punteros.
...
void MyGLWidget::keyPressEvent (QKeyEvent *e) {
  ...
  update ();    //Llama a la función paintGL
}
